...
describe ConversationAiHandler, type: :service do
  ...
  let!(:prompt)                     { create(:prompt) } # Ensure this is correctly set up in your test environment

  describe "#generate_ai_response" do
    before do
      allow(language_service).to receive(:get_embeddings).with(content: content).and_return(question_embedding)
      ...
      allow(conversation_ai_handler).to receive(:generate_ai_response_from_prompt).with(original_text: original_text, content: content, prompt: prompt).and_return(ai_response_content)
    end
